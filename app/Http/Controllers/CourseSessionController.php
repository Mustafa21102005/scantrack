...
class CourseSessionController extends Controller
{
    ...
    public function store(StoreCourseSessionRequest $request)
    {
        ...
        CourseSession::create([
            'qr_token' => $token,
            'expires_at' => $validatedData['expires_at'] ?? now()->addMinutes(10),
            'is_active' => true,
    ...
    {
        if (!$qrCode->is_active) {
            return back()->withErrors(['error' => 'Cannot edit an expired QR Code session.']);
        }

        $validatedData = $request->validated();

        $qrCode->expires_at = $validatedData['expires_at'];
        $qrCode->save();
    ...
    {
        $session = CourseSession::where('qr_token', $token)
